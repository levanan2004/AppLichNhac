...
class _AddCustomerScreenState extends State<AddCustomerScreen> {
  ...
  void _addAutoReminders() {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        title: const Text('Tạo tự động lịch nhắc'),
        content: Text(
          'Bạn có muốn tạo 6 lịch nhắc tự động?\n\n'
          '• ${DateFormat('dd/MM/yyyy').format(baseDate.add(const Duration(days: 3)))} - Sau 3 ngày\n'
          '• ${DateFormat('dd/MM/yyyy').format(baseDate.add(const Duration(days: 7)))} - Sau 7 ngày\n'
          '• ${DateFormat('dd/MM/yyyy').format(baseDate.add(const Duration(days: 15)))} - Sau 15 ngày\n'
          '• ${DateFormat('dd/MM/yyyy').format(DateTime(baseDate.year, baseDate.month + 1, baseDate.day))} - Sau 1 tháng\n'
          '• ${DateFormat('dd/MM/yyyy').format(DateTime(baseDate.year, baseDate.month + 3, baseDate.day))} - Sau 3 tháng\n'
          '• ${DateFormat('dd/MM/yyyy').format(DateTime(baseDate.year, baseDate.month + 6, baseDate.day))} - Sau 6 tháng',
        ),
        actions: [
          TextButton(
  ...
  List<Widget> _buildReminderInputs() {
    return _reminders.asMap().entries.map((entry) {
      ...
      // Tạo title hiển thị với mô tả CSKH
      String getDisplayTitle(int index, String description) {
        if (description.startsWith('Lịch nhắc')) {
          return description; // Đã có mô tả đầy đủ
        }
        return 'Lịch nhắc ${index + 1}'; // Fallback cho reminder thủ công
      }
          ...
          children: [
            Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    getDisplayTitle(index, reminder.description),
                    style: const TextStyle(
                      fontSize: 16,
                      fontWeight: FontWeight.w600,
                      color: Colors.green,
